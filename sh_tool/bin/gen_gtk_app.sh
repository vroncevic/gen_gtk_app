...
function __gen_gtk_app {
    local PN=$1
    if [ -n "${PN}" ]; then
        display_logo "vroncevic" "${GEN_GTK_APP_TOOL}" "${GEN_GTK_APP_VERSION}" "${GEN_GTK_APP_LOGO}"
        local FUNC=${FUNCNAME[0]} MSG="None"
        local STATUS_CONF STATUS_CONF_UTIL STATUS
        MSG="Loading basic and util configuration!"
        info_debug_message "$MSG" "$FUNC" "$GEN_GTK_APP_TOOL"
        progress_bar PB_STRUCTURE
        declare -A config_gen_gtk_app=()
        load_conf "$GEN_GTK_APP_CFG" config_gen_gtk_app
        STATUS_CONF=$?
        declare -A config_gen_gtk_app_util=()
        load_util_conf "$GEN_GTK_APP_UTIL_CFG" config_gen_gtk_app_util
        STATUS_CONF_UTIL=$?
        ...
        if [ $STATUS -eq $NOT_SUCCESS ]; then
            MSG="Force exit!"
            info_debug_message_end "$MSG" "$FUNC" "$GEN_GTK_APP_TOOL"
            exit 129
        fi
        ...
        load_util_conf "${GEN_GTK_APP_HOME}/conf/${PS}" project_set
        STATUS=$?
        if [ $STATUS -eq $NOT_SUCCESS ]; then
            MSG="Force exit!"
            info_debug_message_end "$MSG" "$FUNC" "$GEN_GTK_APP_TOOL"
        ...
        local USRID=${config_gen_gtk_app_util[USERID]}
        local GRPID=${config_gen_gtk_app_util[GROUPID]}
        ...
        logging GEN_GTK_APP_LOGGING
        info_debug_message_end "Done" "$FUNC" "$GEN_GTK_APP_TOOL"
        TREE=$(which tree)
        check_tool "${TREE}"
        STATUS=$?
